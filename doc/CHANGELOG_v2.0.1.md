...
- 新增 **替换翻译模式**：从已翻译图片提取翻译数据并应用到生肉图片
  ...
  - 支持导出可编辑 PSD 文件
  - **新增模板匹配对齐功能**：
    - 启用 `enable_template_alignment` 后，自动计算中日文图的偏移量
    - 使用模板匹配技术找到两张图的对齐位置
    - 自动调整翻译文字的位置，解决图片不完全对齐的问题
    - 可调整模板大小 `template_size`（默认 440 像素）
  - 工作流程：
    1. 准备生肉图和对应的翻译图
    ...
    4. 添加生肉图片
    5. 程序自动从翻译图提取 OCR 结果
    6. （可选）启用模板匹配对齐，自动调整文字位置
    7. 匹配区域并应用翻译
    8. 保存结果到 `manga_translator_work` 目录
