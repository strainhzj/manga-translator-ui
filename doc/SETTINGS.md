...
- **启用模板匹配对齐 (enable_template_alignment)**：启用模板匹配对齐（仅替换翻译模式）
  - 默认：关闭
  - 功能：自动计算中日文图的偏移量，调整翻译文字位置
  - 使用场景：当生肉图和翻译图不完全对齐时（如不同版本、不同分辨率）
  - 工作原理：
    1. 从翻译图中心提取模板
    2. 在生肉图中匹配该模板
    3. 计算水平和垂直偏移量
...
- **模板大小 (template_size)**：模板匹配的模板大小（像素）
  - 默认：440
  - 范围：100-1000
  - 说明：用于模板匹配的图像块大小，越大匹配越精确但速度越慢
  - 建议：保持默认值 440，除非图像尺寸特别小或特别大
